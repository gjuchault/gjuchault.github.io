...
During one week-end (11/20 and 11/21), we participed (along with four members of the  robotics team) to an electronics hackaton. The subject was « Embedded systems serving the environment ».

We were five to participate : two developpers (pretty good in electronics), one project manager and math guy, one specialized in electronic and me (development). Concurrent teams where coming from all over the France, and a majority of the teams were from the hosting school.

## Provided hardware

The organizers provided us WiFi of course, a Raspberry Pi and an AIM Kit (similar to RPi with a touchscreen).  They also had a few activities to let us entertain during the week-end (food, baby-foot, etc.). One of the partner (EDF - which provides electricity/gas all over the Frnance) allowed us to use its Open Data website. The API (REST/JSON) contained some relevant informations.
...
The AIM Kit was used to simulate the data source : sun/wind (two sliders and a send button).![enseak.png]({{site.baseurl}}/_posts/enseak.png)
